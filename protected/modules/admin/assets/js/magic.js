...
function sortGrid(gridId) {
    ...
    if ( $('.handler', grid ).size() > 0 ) {
        opts['handle'] = '.handler';
    }
    grid.sortable(opts).disableSelection();
}



// Плагин переводит значение инпута в транслит и записывает его в назначенное поле
!function($) {

    $.fn.writetranslit = function(dest) {
        if ( dest === undefined ) {
            return;
        }
        if ($.type(dest) == 'string' ) {
        ...
        this.bind({
            keyup: function(e) {
                dest.val( transliterate( $(this).val()) );
                return true;
            },
            change: function(e) {
                dest.val( transliterate( $(this).val()) );
